...
def build_explanation(total_score, components, email):
    ...
    original_sum = components['text'] + components['domain'] + components['semantic']
...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False, user=None):
    ...
    try:
        ...
        # Consulta base para la búsqueda principal
        base_query = {
            "query": {
                "bool": {
                    "must": [],
        ...
        logger.debug("Consulta base creada: %s", json.dumps(base_query, indent=2))
        ...
        # Consulta principal que combina la consulta base con filtros
        es_query = {
            "query": {
                "bool": {
        ...
        # Crear una consulta separada para los filtros "add"
        add_query = {
            "query": {
                "bool": {
        ...
        for filter in add_filters:
            for term in filter.get('terms', []):
                add_query["query"]["bool"]["should"].append({
                    "multi_match": {
                        "query": term,
                        "fields": ["body", "summary", "relevant_terms_array", "subject", "from", "to"],
                        "type": "cross_fields",
            ...
            add_query["query"]["bool"]["minimum_should_match"] = 1

        # Ejecutar la consulta principal
        start_time = datetime.now()
        es_results = es.search(index='email_index', body=es_query)
        logger.debug(f"Búsqueda principal completada en %s segundos", (datetime.now() - start_time).total_seconds())
        total_results = es_results['hits']['total']['value']
        es_hits = es_results['hits']['hits']
        logger.info(f"Resultados de la búsqueda principal: {len(es_hits)} hits, total: {total_results}")

        # Ejecutar la consulta para filtros "add" si existen
        add_hits = []
        if add_filters:
            add_results = es.search(index='email_index', body=add_query)
            logger.debug(f"Búsqueda de filtros add completada en %s segundos", (datetime.now() - start_time).total_seconds())
            add_hits = add_results['hits']['hits']
            logger.info(f"Resultados de filtros add: {len(add_hits)} hits")

        # Combinar resultados de la consulta principal y los filtros "add"
        combined_hits = es_hits + [hit for hit in add_hits if hit['_id'] not in {hit['_id'] for hit in es_hits}]
        logger.debug(f"Resultados combinados: {len(combined_hits)} hits")

        # Conteo para filtros "remove"
        for filter in remove_filters:
            ...
            try:
                ...
                logger.debug(f"Conteo para filtro remove {terms_key}: {count_result['count']} en %s segundos", (datetime.now() - count_start).total_seconds())
            except Exception as e:
                logger.error(f"Error al contar correos para filtro remove {terms_key}: {str(e)}")
                filter_counts['remove'][terms_key] = 0

        # Conteo para filtros "add"
        for filter in add_filters:
            terms_key = ','.join(filter['terms']).lower()
            count_query = {
                "query": {
            ...
            for term in filter['terms']:
                count_query["query"]["bool"]["should"].append({
                    "multi_match": {
                        "query": term,
                        "fields": ["body", "summary", "relevant_terms_array", "subject", "from", "to"],
                        "type": "cross_fields"
            ...
            try:
                ...
                logger.debug(f"Conteo para filtro add {terms_key}: {count_result['count']} en %s segundos", (datetime.now() - count_start).total_seconds())
            except Exception as e:
                logger.error(f"Error al contar correos para filtro add {terms_key}: {str(e)}")
                filter_counts['add'][terms_key] = 0
        ...
        total_filtered_results = total_results + sum(filter_counts['add'].values()) if add_filters else total_results
