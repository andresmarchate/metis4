...
def clear_cache(query_hash):
    logger.info("Eliminando caché para query_hash: %s", query_hash)
    try:
        deleted = redis_client.delete(query_hash)
        if deleted:
            logger.debug("Caché eliminado exitosamente para: %s", query_hash)
        else:
            logger.debug("No se encontró caché para eliminar: %s", query_hash)
    except redis.RedisError as e:
        logger.error("Error al eliminar caché: %s", str(e), exc_info=True)