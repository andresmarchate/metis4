...
def get_dashboard_metrics():
    ...
    try:
        ...
        for period_name, period_info in periods.items():
            ...
            # Base match criteria for received emails with date filter
            base_match_received = {
                'mailbox_id': {'$in': user_mailboxes},
                'to': {'$regex': '|'.join([f'\\b{re.escape(mailbox)}\\b' for mailbox in user_mailboxes]), '$options': 'i'},
                'date': {'$ne': 'unknown'},  # Excluir fechas inválidas
                '$expr': {
                    '$gte': [{'$toDate': '$date'}, start_date]
                }
            }

            # Contar documentos con fecha inválida para received
            invalid_date_count_received = emails_collection.count_documents({
                'mailbox_id': {'$in': user_mailboxes},
                'date': 'unknown'
            })
            logger.debug(f"Documentos con fecha inválida para received en {period_name}: {invalid_date_count_received}")

            # Received emails
            received_count = emails_collection.count_documents(base_match_received)
            metrics['received'][period_name] = received_count

            # Base match criteria for sent emails with date filter
            base_match_sent = {
                'mailbox_id': {'$in': user_mailboxes},
            ...
            # Contar documentos con fecha inválida para sent
            invalid_date_count_sent = emails_collection.count_documents({
                'mailbox_id': {'$in': user_mailboxes},
                'date': 'unknown'
            })
            logger.debug(f"Documentos con fecha inválida para sent en {period_name}: {invalid_date_count_sent}")

            # Sent emails
            sent_count = emails_collection.count_documents(base_match_sent)
            metrics['sent'][period_name] = sent_count

            # Top 10 senders with date filter
            senders_pipeline = [
                {'$match': {'date': {'$ne': 'unknown'}}},  # Excluir fechas inválidas
            ...
            # Top 10 recipients with date filter
            recipients_pipeline = [
                {'$match': {'date': {'$ne': 'unknown'}}},  # Excluir fechas inválidas
            ...
            # Classification counts (only for received emails) with date filter
...
def get_thread_emails(todo_id):
    ...
    try:
        ...
        if not message_id:
            logger.error(f"No message_id found for TODO: {todo_id}")
            return {'emails': []}
        
        # Obtener el correo original asociado al message_id
        email = emails_collection.find_one({"message_id": message_id})
        if not email:
            logger.warning(f"Email not found for message_id: {message_id}, attempting fallback")
        else:
            logger.info(f"Found original email for message_id: {message_id}")
        
        # Intentar obtener el hilo completo
        thread_emails = []
        if email and email.get('parent_thread_id'):
            # Usar parent_thread_id para encontrar el hilo
            match_criteria = {'parent_thread_id': email['parent_thread_id'], 'date': {'$ne': 'unknown'}}  # Excluir fechas inválidas
            logger.info(f"Using parent_thread_id: {email['parent_thread_id']} to fetch thread")
        else:
            # Fallback a in_reply_to o message_id
            logger.info(f"No parent_thread_id, falling back to message_id: {message_id}")
            match_criteria = {
                '$or': [
                    {'message_id': message_id},
                    {'in_reply_to': message_id}
                ],
                'mailbox_id': todo['mailbox_id'],
                'date': {'$ne': 'unknown'}  # Excluir fechas inválidas
            }
            if email and email.get('in_reply_to'):
                match_criteria['$or'].append({'message_id': email['in_reply_to']})
        
        # Buscar correos del hilo
        pipeline = [
            {'$match': match_criteria},
            {
                '$project': {
                    ...
                    'requires_response': 1,
                    'responded': 1,
                    'message_id': 1  # Añadido para identificar duplicados
                }
            },
            {'$sort': {'date': 1}}
        ]
        
        thread_emails = list(emails_collection.aggregate(pipeline))
        logger.info(f"Found {len(thread_emails)} emails in initial thread search")
        
        # Si no se encuentra el hilo, intentar con coincidencia de asunto
        if not thread_emails and email:
            cleaned_subject = re.sub(r'^(Re:|Fwd:)\s*', '', email.get('subject', ''), flags=re.IGNORECASE).strip()
            match_criteria = {
                'subject': {'$regex': f"^(Re:|Fwd:)?\s*{re.escape(cleaned_subject)}", '$options': 'i'},
                'mailbox_id': todo['mailbox_id'],
                'date': {'$ne': 'unknown'}  # Excluir fechas inválidas
            }
            pipeline[0] = {'$match': match_criteria}
            thread_emails = list(emails_collection.aggregate(pipeline))
            logger.info(f"Fallback to subject match found {len(thread_emails)} emails")
        
        # Asegurarse de incluir el correo original si existe
        if email and not any(e.get('message_id') == message_id for e in thread_emails):
            formatted_email = {
                'index': str(email.get('index', 'N/A')),
                'from': format_email_field_with_fallback(email.get('from', ''), email.get('from_email', '')),
                'to': format_email_field_with_fallback(email.get('to', ''), email.get('to_email', '')),
                'subject': email.get('subject', ''),
                'date': email.get('date', ''),
                'summary': email.get('summary', ''),
                'urgent': email.get('urgent', False),
                'important': email.get('important', False),
                'advertisement': email.get('advertisement', False),
                'requires_response': email.get('requires_response', False),
                'responded': email.get('responded', False),
                'message_id': email.get('message_id', '')
            }
            thread_emails.append(formatted_email)
            logger.info(f"Added original email to thread_emails")
        
        # Eliminar duplicados basados en message_id
        seen_message_ids = set()
        unique_thread_emails = []
        for email in thread_emails:
            message_id = email.get('message_id')
            if message_id and message_id not in seen_message_ids:
                seen_message_ids.add(message_id)
                unique_thread_emails.append(email)
            elif not message_id:
                # Si no tiene message_id, incluirlo pero advertir
                logger.warning(f"Email without message_id found in thread: {email.get('index', 'N/A')}")
                unique_thread_emails.append(email)
        thread_emails = unique_thread_emails
        
        # Formatear los correos del hilo
        for email in thread_emails:
            email['index'] = str(email.get('index', 'N/A'))
            email['from'] = format_email_field_with_fallback(email.get('from', ''), email.get('from_email', ''))
            to_field = email.get('to', '')
        ...
        logger.info(f"Fetched {len(thread_emails)} unique emails for thread related to TODO: {todo_id}")
        return {'emails': thread_emails}
    except Exception as e:
