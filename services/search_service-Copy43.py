...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False, user=None):
    ...
    try:
        ...
        for hit in es_hits:
            ...
            logger.debug(f"Explicación de Elasticsearch para message_id {hit['_source']['message_id']}: {explanation}")
            ...
            logger.debug(f"Detalles extraídos para message_id {hit['_source']['message_id']}: text_details={text_details}, domain_score={domain_score}, semantic_score={semantic_score}")
            
            # Calcular la puntuación de texto sumando las puntuaciones de los detalles de texto
            text_score = sum(detail['score'] for detail in text_details)
        ...
        for email in results:
            ...
            if relevance >= min_relevance or filter_only:
                # Construir la explicación detallada
                ...
                if email['text_details']:
                    ...
                    terms = set(detail['term'] for detail in email['text_details'])
                    explanation += f"\"{', '.join(terms)}\" en campos como "
                    
                    # Agrupar por campo para resumir
                    field_scores = {}
                    for detail in email['text_details']:
                        field = detail['field']
                        if field not in field_scores:
                            field_scores[field] = 0.0
                        field_scores[field] += detail['score']
                ...
                # Añadir puntuación semántica si existe
                ...
                # Añadir mensaje predeterminado si no hay detalles específicos
                if not email['text_details'] and email['domain_score'] == 0 and email['semantic_score'] == 0:
                ...
                logger.debug(f"Explicación construida para message_id {email['message_id']}: {explanation}")
...
def extract_scores(explanation, text_details=None, domain_score=0.0, semantic_score=0.0):
    """Extrae detalles de texto, puntuación de dominio y semántica de la explicación de Elasticsearch."""
    if text_details is None:
        text_details = []
    
    if 'value' in explanation and explanation['value'] > 0:
        desc = explanation.get('description', '')
        logger.debug(f"Procesando descripción de explicación: {desc}")
        
        # Buscar coincidencias de texto
        if 'weight' in desc and ('multi_match' in desc or 'match' in desc):
            ...
            if match:
                ...
                logger.debug(f"Coincidencia de texto encontrada: field={field}, term={term}, score={explanation['value']}")
        
        # Buscar puntuación de dominio
        elif 'terms' in desc:
            domain_score += explanation['value']
            logger.debug(f"Puntuación de dominio encontrada: score={explanation['value']}")
        
        # Buscar puntuación semántica
        elif 'script_score' in desc:
            semantic_score += explanation['value']
            logger.debug(f"Puntuación semántica encontrada: score={explanation['value']}")
        
        # Recursar en detalles anidados
        for detail in explanation.get('details', []):
            t_details, d_score, s_score = extract_scores(detail, text_details, domain_score, semantic_score)
            text_details.extend(t_details)
