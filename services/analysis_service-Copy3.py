...
def generate_llm_summary(email):
    ...
    text = normalize_text(text)[:5000]  # Truncate to ~1000 chars
    if not text:
        logger.warning(f"Texto vacío para correo: {email['message_id']}")
        return generate_tfidf_summary([text])

    prompt = f"""
    Resuma este correo en 1-2 frases en español que sean explicativas, no un resumen escueto que no aporte informacion, enfocándose en el tema principal e intención. 
...
def analyze_themes(email_ids):
    ...
    try:
        ...
        n_clusters = min(max(2, int(np.sqrt(len(valid_emails)))), 5)  # At least 2 clusters
        logger.debug(f"Usando {n_clusters} clústeres para {len(valid_emails)} correos")
        ...
        themes = {}
        for label, email, text in zip(labels, valid_emails, texts):
