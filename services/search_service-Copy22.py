...
def get_email_by_id(identifier, is_index=False):
    ...
    try:
        ...
        if email['index'] == 'N/A' or not email['index']:
            logger.warning("Campo 'index' inválido en correo: %s=%s, usando message_id como respaldo", query_field, identifier)
            email['index'] = email['message_id'] if email['message_id'] != 'N/A' else 'N/A'
...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False):
    ...
    try:
        ...
        if filter_only:
            ...
            for result in filter_results:
                ...
                if result['index'] == 'N/A' or not result['index']:
                    logger.warning("Campo 'index' inválido en correo: message_id=%s", result.get('message_id', 'N/A'))
                    result['index'] = result['message_id'] if result['message_id'] != 'N/A' else 'N/A'
                if result['message_id'] == 'N/A' or not result['message_id']:
                    logger.warning("Campo 'message_id' inválido en correo: index=%s", result.get('index', 'N/A'))
                    result['message_id'] = result['index'] if result['index'] != 'N/A' else 'N/A'
        ...
        for email in results:
            ...
            email['message_id'] = str(email.get('message_id', 'N/A'))
            if email['index'] == 'N/A' or not email['index']:
                logger.warning("Campo 'index' inválido en correo: message_id=%s", email.get('message_id', 'N/A'))
                email['index'] = email['message_id'] if email['message_id'] != 'N/A' else 'N/A'
            if email['message_id'] == 'N/A' or not email['message_id']:
                logger.warning("Campo 'message_id' inválido en correo: index=%s", email.get('index', 'N/A'))
                email['message_id'] = email['index'] if email['index'] != 'N/A' else 'N/A'
        ...
        for idx, email in enumerate(results, start=1):
            ...
            logger.debug("Email formatted: from=%s, to=%s, index=%s, message_id=%s", email['from'], email['to'], email['index'], email['message_id'])
