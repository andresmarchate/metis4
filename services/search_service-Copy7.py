...
def get_email_by_id(message_id):
    ...
    try:
        ...
        logger.debug("Processing from: from_field=%s, from_email=%s", from_field, from_email) # Depuración
        ...
        logger.debug("Processing to: to_field=%s, to_email=%s", to_field, to_email) # Depuración
        ...
        logger.debug("Correo procesado: from=%s, to=%s", email['from'], email['to']) # Depuración
        return email
    except Exception as e:
...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25):
    ...
    try:
        ...
        for idx, email in enumerate(results, start=1):
            ...
            from_email = email.get('from_email', '') or extract_email(from_field)
            to_email = email.get('to_email', '') or extract_email(to_field)
            logger.debug("Processing email fields: from_field=%s, from_email=%s, to_field=%s, to_email=%s", from_field, from_email, to_field, to_email) # Depuración
            ...
            logger.debug("Email formatted: from=%s, to=%s", email['from'], email['to']) # Depuración
