...
def extract_components(explanation):
    ...
    recurse(explanation)
    logger.debug(f"Componentes extra Detox: text={components['text']}, domain={components['domain']}, semantic={components['semantic']}, text_details={components['text_details']}")
    return components
...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False, user=None):
    ...
    try:
        ...
        # Conteo para filtros remove usando la consulta base más los términos del filtro
        for filter in remove_filters:
...
def get_filter_emails(query_terms, filter, user):
    logger.info("Obteniendo correos para filtro: %s con términos de consulta: %s, user: %s", filter, query_terms, user.username if user else 'None')
    if not user:
    ...
    es_query = {
        ...
        "size": 1000  # Límite razonable para la ventana modal
    }
    ...
    try:
        ...
        for hit in hits:
            results.append({
                'index': str(hit['_source'].get('index', 'N/A')),
                'message_id': str(hit['_source'].get('message_id', 'N/A')),
                'date': hit['_source'].get('date', ''),
                'from': hit['_source'].get('from', 'N/A'),
                'to': hit['_source'].get('to', 'N/A'),
                'subject': hit['_source'].get('subject', ''),
                'description': hit['_source'].get('summary', 'Sin resumen'),
                'relevant_terms': hit['_source'].get('relevant_terms_array', [])
            })
