...
def analyze_themes(email_ids, user):
    ...
    try:
        ...
        valid_emails = []
        for email in emails:
            subject = email.get('subject', '') * 2  # Ponderación ajustada
            sender = email.get('from', '') * 1
            summary = email.get('summary', '') * 1
            body_excerpt = email.get('body', '')[:200] * 2  # Más peso al cuerpo
            text = f"{subject} {sender} {summary} {body_excerpt}"
        ...
        min_clusters = 3
        max_clusters = min(max(3, int(n_emails / 3)), 50)  # Más clusters
        ...
        similarities = [util.cos_sim(embedding, centroids[label]).item() for embedding, label in zip(embeddings, labels)]
        similarity_threshold = np.percentile(similarities, 10)  # Umbral más bajo
        ...
        # Filtrar temas con menos de 2 correos y mover a "Otros"
        others = {'emails': [], 'texts': []}
        filtered_themes = {}
        for label, data in themes.items():
            if len(data['emails']) >= 2:
                filtered_themes[label] = data
            else:
                others['emails'].extend(data['emails'])
                others['texts'].extend(data['texts'])

        if others['emails']:
            filtered_themes['others'] = others

        result = []
        for label, data in filtered_themes.items():
