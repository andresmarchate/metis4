...
# Version: o5p6q7r8-s9t0-u1v2-w3x4-y5z6a7b8c9
import unicodedata
import zlib
import numpy as np
...
def get_email_by_id(identifier, is_index=False):
    ...
    try:
        ...
        to_match = re.match(r'(.*)<(.+)>', from_field) if isinstance(from_field, str) else None
        if to_match:
        ...
        if filter_only:
            ...
            pipeline = [
                ...
                {'$project': {
                    ...
                    'semantic_domain': 1,
                    'embedding': 1,
                    'score': {'$meta': 'textScore'} if text_query else ''
                }}
            ]
