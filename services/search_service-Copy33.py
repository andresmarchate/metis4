...
# Diccionario de sinónimos para dominios semánticos (extensible)
DOMAIN_SYNONYMS = {
    "informacion_juridica": ["legal", "derecho", "judicial"],
    "viajes": ["turismo", "vacaciones", "reserva"],
    ...
    "general": []  # Sin sinónimos por defecto
    # Añade más dominios y sinónimos según necesites
}
...
def cosine_similarity(emb1, emb2, compressed=True):
    """Calcula la similitud coseno entre dos embeddings. Si compressed=True, descomprime emb1 y emb2."""
...
def extract_email_from_input(input_str):
    """Extrae dirección de correo de una entrada de autocompletado (e.g., 'ANDRES <andres.m.tirado@gmail.com>')."""
...
def get_email_addresses(prefix='', limit=50, user=None):
    """Obtiene direcciones de correo únicas de 'from' y 'to' para autocompletado, filtradas por buzones del usuario."""
...
def get_conversation_emails(email1, email2, start_date, end_date, user=None):
    """Obtiene correos bidireccionales entre email1 y email2 en un rango de fechas, filtrados por buzones del usuario."""
...
def get_email_by_id(identifier, is_index=False):
    ...
    logger.debug("Preparando para obtener correo con identifier: %s, is_index: %s", identifier, is_index)
...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False, user=None):
    """
    Busca correos filtrados por los buzones del usuario autenticado usando Elasticsearch y refinamiento con MongoDB.
    ...
    try:
        ...
        es_query = {
            "query": {
                "bool": {
                    "should": [],
                    "filter": [
                        {"terms": {"mailbox_id": user_mailboxes}}
                    ],
        ...
        # Añadir términos de búsqueda como cláusulas 'should'
        if terms and not filter_only:
            for term in terms:
                es_query['query']['bool']['should'].append({
                    "multi_match": {
                        "query": term,
                        "fields": ["body", "summary", "relevant_terms_array", "subject"],
                        "type": "cross_fields",
                        "operator": "and"
                    }
                })

        # Añadir filtro de dominio semántico incluyendo sinónimos
        if intent != 'general' and not filter_only:
            related_domains = DOMAIN_SYNONYMS.get(intent, [intent])
            es_query['query']['bool']['filter'].append({
                "terms": {"semantic_domain": related_domains}
            })

        # Añadir filtros de metadatos
        metadata_filters = processed_query.get('metadata_filters', {})
        if 'from' in metadata_filters:
            es_query['query']['bool']['filter'].append({
                "match": {"from": metadata_filters['from']}
            })
        if 'to' in metadata_filters:
            es_query['query']['bool']['filter'].append({
                "match": {"to": metadata_filters['to']}
            })
        if 'subject' in metadata_filters:
            es_query['query']['bool']['filter'].append({
                "match": {"subject": metadata_filters['subject']}
            })
        if 'date_range' in metadata_filters:
            es_query['query']['bool']['filter'].append({
                "range": {
                    "date": {
                        "gte": metadata_filters['date_range']['start'],
                        "lte": metadata_filters['date_range']['end']
                    }
                }
            })

        # Añadir búsqueda vectorial como cláusula 'should'
        ...
        # Si es filter_only, ajustar la consulta para filtros específicos
        if filter_only:
            if not filters:
        ...
        # Verificar calidad de resultados y fallback a MongoDB si es necesario
        THRESHOLD = 0.1  # Umbral reducido para depuración
        if not es_hits or (max(hit['_score'] for hit in es_hits if '_score' in hit) < THRESHOLD):
            logger.info("Resultados de Elasticsearch de baja calidad o vacíos, usando MongoDB como fallback")
            text_query = {
                '$text': {
                    '$search': ' '.join(terms),
                    '$language': 'spanish'
            ...
            # Recuperar documentos completos desde MongoDB
            ...
            # Añadir puntuación y embedding de Elasticsearch a los resultados
        ...
        # Añadir correos de filtros de inclusión
        ...
        # Calcular relevancia con puntuación textual y semántica
        ranked_results = []
        relevance_scores = []
        semantic_scores = []
        text_scores = []
        ...
        for email in filtered_results:
            # Usar embedding de Elasticsearch si está disponible, si no, descomprimir el de MongoDB
        ...
        for email in filtered_results:
            ...
            relevance = int((0.4 * normalized_semantic_score + 0.5 * normalized_text_score + 0.1 * feedback_weight) * 100)
            ...
            if 'semantic_domain' in email and email['semantic_domain'] in DOMAIN_SYNONYMS.get(intent, [intent]) and not filter_only:
                explanation_parts.append(f"dominio semántico '{email['semantic_domain']}'")
