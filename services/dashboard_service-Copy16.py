...
users_collection = db[MONGO_USERS_COLLECTION]  # Nueva colección añadida
...
def get_agatta_todos(username):
    """Fetch AGATTA todos for the specified user."""
    logger.info("Fetching AGATTA todos for user: %s", username)
    try:
        ...
        todos = list(todos_collection.find(query))
        logger.info(f"Encontrados {len(todos)} TODOs")
