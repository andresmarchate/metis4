...
def create_draft(service, user_id, email, proposed_action, metadata=None):
    ...
    try:
        ...
        # Configurar threadId si está disponible en metadata o en el email original
        thread_id = None
        ...
        logger.error(f"Error creating Gmail draft: {error}")
        raise
    except Exception as e:
        logger.error(f"Unexpected error creating Gmail draft: {str(e)}", exc_info=True)
        raise