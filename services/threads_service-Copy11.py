...
def analyze_threads(query, user=None):
    ...
    try:
        ...
        logger.debug(f"Type of query_embedding after decompression: {type(query_embedding_decompressed)}, dimension: {len(query_embedding_decompressed)}")
        if len(query_embedding_decompressed) != 384:
            logger.error(f"Dimensión de query_embedding descomprimido incorrecta: {len(query_embedding_decompressed)} (esperado: 384)")
            raise ValueError("Dimensión de query_embedding no coincide con el modelo MiniLM-L12-v2")
        ...
        logger.debug(f"Shape of query_embedding_np: {query_embedding_np.shape}")
        logger.debug(f"Shape of email_embeddings_np: {email_embeddings_np.shape}")
        ...
        # Filter emails with similarity above threshold or containing key terms
        similarity_threshold = 0.4  # Reducido para ser más inclusivo
        term_pattern = re.compile('|'.join(flat_terms), re.IGNORECASE)
        filtered_indices = []
        for i, email in enumerate(emails):
            text = f"{email.get('subject', '')} {email.get('summary', '')} {email.get('body', '')}"
            if similarities[i] > similarity_threshold or term_pattern.search(text):
                filtered_indices.append(i)
        filtered_emails = [emails[i] for i in filtered_indices]
        filtered_embeddings = email_embeddings_np[filtered_indices]
        logger.debug(f"Assigned filtered_embeddings with shape: {filtered_embeddings.shape}")
        logger.debug(f"Filtered to {len(filtered_emails)} emails with similarity > {similarity_threshold} or containing key terms")

        if not filtered_emails:
            logger.warning("No emails passed the filtering criteria")
            return []
    ...
    try:
        ...
        for i in range(len(emails)):
            for j in range(i + 1, len(emails)):
                ...
                logger.debug(f"Parsed dates: date_i={date_i}, date_j={date_j}")
        ...
        logger.debug(f"Passing labels to merge_small_threads: {len(labels)} labels")
...
def merge_small_threads(threads, embeddings, labels):
    """Merge small threads with high similarity to improve coherence."""
    logger.debug(f"Received labels in merge_small_threads: {len(labels)} labels, unique: {len(set(labels))}")
    formatted_threads = []
