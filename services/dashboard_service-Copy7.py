...
# Version: a2b3c4d5-e6f7-8901-a4b5-c6d7e8f9g0
import logging
...
def get_email_list(metric, period, sender=None, recipient=None):
    ...
    try:
        ...
        if sender:
            match_criteria['from'] = {'$regex': f'^{re.escape(sender)}$', '$options': 'i'}
        if recipient:
            match_criteria['to'] = {'$regex': f'\\b{re.escape(recipient)}\\b', '$options': 'i'}
        ...
        logger.debug("Raw email list data: %s", [{k: v for k, v in email.items() if k in ['from', 'to', 'from_email', 'to_email']} for email in emails])
        ...
        for email in emails:
            ...
            logger.debug("Formatted email: from=%s, to=%s", email['from'], email['to'])
