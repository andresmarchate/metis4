...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False, user=None):
    ...
    try:
        ...
        for hit in es_hits:
            ...
            text_score, domain_score, semantic_score = extract_scores(explanation)
        ...
        for email in results:
            ...
            if relevance >= min_relevance or filter_only:
                explanation_parts = []
                if email['text_score'] > 0:
                    explanation_parts.append(f"coincidencia textual: {email['text_score']:.2f}")
                if email['domain_score'] > 0:
                    explanation_parts.append(f"dominio semántico: {email['domain_score']:.2f}")
                if email['semantic_score'] > 0:
                    explanation_parts.append(f"similitud semántica: {email['semantic_score']:.2f}")
                explanation = f"Seleccionado por {', '.join(explanation_parts)} (puntuación total: {email['total_score']:.2f})" if explanation_parts else f"Seleccionado por puntuación total: {email['total_score']:.2f}"
...
def extract_scores(explanation, text_score=0.0, domain_score=0.0, semantic_score=0.0):
    """Extrae las puntuaciones de texto, dominio y semántica de la explicación de Elasticsearch."""
    if 'value' in explanation and explanation['value'] > 0:
        desc = explanation.get('description', '')
        if 'weight' in desc:
            if 'multi_match' in desc or 'match' in desc:
                text_score += explanation['value']
            elif 'terms' in desc:
                domain_score += explanation['value']
            elif 'script_score' in desc:
                semantic_score += explanation['value']
        elif 'sum of' in desc or 'max of' in desc or 'product of' in desc:
            for detail in explanation.get('details', []):
                t, d, s = extract_scores(detail)
                text_score += t
                domain_score += d
                semantic_score += s
    return text_score, domain_score, semantic_score