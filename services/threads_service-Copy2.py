...
# Version: f8g9h0i1-j2k3-l4m5-n6o7-p8q9r0s1
import logging
...
def analyze_threads(query, user_id=None):
    ...
    try:
        ...
        # Process query
        processed_query, intent, terms, query_embedding, names = process_query(query)
...
def fetch_relevant_emails(query, terms, names, query_embedding):
    ...
    try:
        ...
        pipeline = [
            {
                '$match': {
                    '$or': [
                        ...
                        {'from': {'$regex': '|'.join(names), '$options': 'i'}} if names else {},
                        {'to': {'$regex': '|'.join(names), '$options': 'i'}} if names else {}
                    ]
                }
            },
            {
        ...
        emails = list(emails_collection.aggregate([p for p in pipeline if p]))
        logger.debug(f"Fetched {len(emails)} candidate emails")
        return emails
    except Exception as e:
...
def process_feedback(email_index, query, action, user_id):
    ...
    try:
        ...
        # Extract features
        processed_query, _, terms, _, names = process_query(query)
        ...
        feature_importance = np.abs(bayesian_filter.coef_[0])
