...
def analyze_themes(email_ids, user):
    ...
    try:
        ...
        for label, data in themes.items():
            ...
            centroid = kmeans.cluster_centers_[label] if 'kmeans' in locals() else theme_embeddings.mean(axis=0)
        ...
        logger.info(f"Analysis completed: {len(result)} themes identified with {sum(len(t['emails']) for t in result)} emails")
        return result
    except Exception as e:
