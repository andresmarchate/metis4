...
def get_email_by_id(identifier, is_index=False):
    ...
    try:
        ...
        if 'index' not in email:
            logger.warning("Campo 'index' faltante en correo: %s=%s", query_field, identifier)
            email['index'] = 'N/A'
...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False):
    ...
    try:
        ...
        for email in results:
            ...
            if 'index' not in email:
                logger.warning("Campo 'index' faltante en correo: message_id=%s", email['message_id'])
                email['index'] = 'N/A'
