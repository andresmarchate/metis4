...
# Version: q2r3s4t5-u6v7-8901-q4r5-s6t7u8v9w0
import logging
...
def get_dashboard_metrics():
    ...
    try:
        ...
        # Regex to validate date strings (e.g., "Fri, 30 May 2025 13:03:12 +0200")
        date_pattern = re.compile(r'^(?:\w{3}, )?\d{1,2} \w{3} \d{4} \d{2}:\d{2}:\d{2} (?:[+-]\d{4}|\w{3,4})$')

        for period_name, start_date in periods.items():
            # Base match criteria with date validation
            base_match = {
            ...
            # Log invalid dates
            invalid_dates = list(emails_collection.find({
                'to': {'$regex': f'\\b{re.escape(USER_EMAIL)}\\b', '$options': 'i'},
                'date': {'$not': {'$regex': date_pattern}}
            }, {'message_id': 1, 'date': 1}).limit(10))
            if invalid_dates:
                logger.warning(f"Found {len(invalid_dates)} emails with invalid date formats: {[d['message_id'] + ': ' + d['date'] for d in invalid_dates]}")
