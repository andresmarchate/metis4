...
def analyze_threads(query, user=None):
    ...
    try:
        ...
        scored_emails = score_emails(emails, flat_terms, names, query)
        ...
        if not top_emails:
            logger.warning("No emails passed relevance threshold for user")
            return []
...
def fetch_relevant_emails(query, terms, names, query_embedding, user):
    """
    Fetch emails from MongoDB with multi-factor scoring, filtered by user's mailboxes.
    """
...
def score_content_similarity(email1, email2, vectorizer):
    ...
    logger.debug(f"Scoring content similarity between email1 index: {email1.get('index', 'N/A')} and email2 index: {email2.get('index', 'N/A')}")
    ...
    logger.debug(f"Text1: {text1[:50]}..., Text2: {text2[:50]}...")
    vectors = vectorizer.transform([text1, text2])
    similarity = cosine_similarity(vectors[0], vectors[1])[0][0]
    logger.debug(f"Content similarity score: {similarity}")
    return similarity
...
def generate_thread_label(emails):
    ...
    """
    try:
        # Collect original and normalized subjects
        ...
        # Filter non-empty normalized subjects
        ...
        if valid_indices:
            ...
            if len(unique_norm_subjs) == 1:
                # All subjects are the same, use the original subject
                base_title = original_subjects[valid_indices[0]]
            else:
                # Multiple subjects, compute dominant terms
                subject_vectorizer = TfidfVectorizer(max_features=500, stop_words='english')
                subject_vectors = subject_vectorizer.fit_transform([normalized_subjects[i] for i in valid_indices])
                feature_names = subject_vectorizer.get_feature_names_out()
                top_indices = np.argsort(subject_vectors.sum(axis=0).A1)[-3:]
                top_terms = [feature_names[i] for i in top_indices]
                
                # Add terms from relevant_terms
                all_terms = []
                for email in emails:
                ...
                base_title = "Hilo: " + ", ".join(unique_terms)
        else:
            # No valid subjects, use dominant terms from relevant_terms
            all_terms = []
            for email in emails:
                all_terms.extend(email.get('relevant_terms', []))
            from collections import Counter
