...
def get_outbox_count(username):
    ...
    for mailbox in active_mailboxes:
        ...
        if service:
            try:
                # En Gmail, los correos no enviados se consideran borradores
                drafts = service.users().drafts().list(userId='me').execute()
...
def get_draft_emails(username):
    logger.debug(f"Fetching draft emails for user: {username}")
    user = users_collection.find_one({"username": username})
    if not user:
        logger.warning(f"User not found: {username}")
        return []
        ...
        if service:
            try:
                ...
                for draft in drafts.get('drafts', []):
                    ...
                    email_data = {
                        'index': draft['id'],
                        'from': 'Yo',  # Los borradores son del usuario
                    ...
                    logger.debug(f"Draft email added: {draft['id']}")
            except Exception as e:
                logger.error(f"Error fetching draft emails for mailbox {mailbox['mailbox_id']}: {str(e)}")
    logger.info(f"Fetched {len(draft_emails)} draft emails for user {username}")
    return draft_emails

def get_outbox_emails(username):
    logger.debug(f"Fetching outbox emails for user: {username}")
    # En Gmail, los correos no enviados son borradores, así que reutilizamos get_draft_emails
    return get_draft_emails(username)