...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False, user=None):
    ...
    try:
        ...
        filters = filters or []
        remove_filters = [f['terms'] for f in filters if f.get('action') == 'remove']
        add_filters = [f['terms'] for f in filters if f.get('action') == 'add']
        filter_counts = {
        ...
        THRESHOLD = 0.1
        if not es_hits or max([hit['_score'] for hit in es_hits], default=0) < THRESHOLD:
            logger.info("Resultados insuficientes de Elasticsearch, usando MongoDB como fallback")
            text_query = {
                '$text': {
                    '$search': ' '.join(terms),
                    '$language': 'spanish'
            ...
            logger.info(f"Resultados de MongoDB: {len(results)} documentos, total: {total_results}")
        else:
            results = [
                {
        ...
        ranked_results = []
        max_score = max([hit.get('es_score', 0) for hit in results], default=1)
        for email in results:
            text_score = email.get('es_score', 0) if 'es_score' in email else 0
            relevance = int((text_score / max_score) * 100) if max_score > 0 else 0
            relevant_terms = email.get('relevant_terms', email.get('relevant_terms_array', []))
            if relevance >= min_relevance or filter_only:
                ranked_results.append({
                    ...
                    'relevance': relevance,
                    'es_score': text_score,
                    'explanation': f"Seleccionado por coincidencia textual y semántica (puntuación: {text_score:.2f})"
                })
        ...
        for filter in filters:
            ...
            if action == 'remove':
                ...
                filter_counts['add'][terms_key] = sum(
                    1 for email in ranked_results
                    if any(term.lower() in email['subject'].lower() or
                           term.lower() in email['description'].lower() or
                           any(term.lower() in rt.lower() for rt in email['relevant_terms'])
                           for term in filter_terms)
                )
