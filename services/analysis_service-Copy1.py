...
from services.nlp_service import normalize_text
...
def generate_summary(texts):
    """Generate a simple summary by selecting the first sentence of the most representative text."""
    if not texts:
        return "No hay contenido suficiente para generar un resumen."
    # Choose the longest text as representative
    main_text = max(texts, key=len, default="")
    sentences = re.split(r'[.!?]+', main_text)
    return sentences[0].strip() if sentences and sentences[0].strip() else "Sin resumen disponible."
...
def determine_status(emails):
    ...
    recent_threshold = '2025-05-01'  # Assume recent if after this date
    for email in emails:
...
def analyze_themes(email_ids):
    ...
    try:
        ...
        # Determine number of clusters (min 3, max 5 or sqrt(n_emails))
        ...
        for label, data in themes.items():
            ...
            theme = {
                ...
                'summary': generate_summary(theme_texts),
