...
def extract_components(explanation):
    ...
    def recurse(expl):
        ...
        if 'value' in expl and expl['value'] > 0:
            ...
            logger.debug(f"Procesando: {desc}, value={expl['value']}")
            ...
            if 'weight' in desc:
                ...
                if match:
                    ...
                    logger.debug(f"Texto: field={field}, term={term}, score={score}")
    ...
    recurse(explanation)
    logger.debug(f"Componentes extraídos: {components}")
    return components
...
def build_explanation(total_score, components, email):
    ...
    if not contributions:
        ...
        explanation += '\n'.join(contributions) + '\n'
        explanation += f"**Suma total de componentes: {sum([components['text'], components['domain'], components['semantic']]):.2f}**, ajustada a {total_score:.2f} por normalización de Elasticsearch."
