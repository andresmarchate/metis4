...
# Version: x3y4z5a6-b7c8-d9e0-f1g2-h3i4j5k6l7
import unicodedata
import zlib
import numpy as np
...
def get_conversation_emails(sender, recipient, start_date, end_date):
    """Fetch emails between sender and recipient within a date range."""
    logger.info(f"Buscando correos entre sender: {sender}, recipient: {recipient}, desde {start_date} hasta {end_date}")
    try:
        sender_email = extract_email_from_input(sender)
        recipient_email = extract_email_from_input(recipient)
        if not sender_email or not recipient_email:
            logger.warning("Dirección de correo inválida: sender=%s, recipient=%s", sender, recipient)
            return []
        ...
        sender_email_escaped = re.escape(sender_email)
        recipient_email_escaped = re.escape(recipient_email)
        ...
        pipeline = [
            {
                '$match': {
                    '$or': [
                        {
                            'from': {'$regex': f'\\b{sender_email_escaped}\\b', '$options': 'i'},
                            'to': {'$regex': f'\\b{recipient_email_escaped}\\b', '$options': 'i'}
                        },
                        {
                            'from': {'$regex': f'\\b{recipient_email_escaped}\\b', '$options': 'i'},
                            'to': {'$regex': f'\\b{sender_email_escaped}\\b', '$options': 'i'}
                        }
                    ]
                }
            },
        ...
        if not emails:
            ...
            sample_emails = list(emails_collection.find(
                {'$or': [
                    {'from': {'$regex': sender_email_escaped, '$options': 'i'}},
                    {'to': {'$regex': sender_email_escaped, '$options': 'i'}}
                ]},
                {'from': 1, 'to': 1, 'date': 1}
            ).limit(5))
            logger.debug(f"Muestra de correos para sender: {sample_emails}")
            sample_emails = list(emails_collection.find(
                {'$or': [
                    {'from': {'$regex': recipient_email_escaped, '$options': 'i'}},
                    {'to': {'$regex': recipient_email_escaped, '$options': 'i'}}
                ]},
                {'from': 1, 'to': 1, 'date': 1}
            ).limit(5))
            logger.debug(f"Muestra de correos para recipient: {sample_emails}")
...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False):
    ...
    try:
        ...
        filter_counts = {
            'remove': {tuple(term): 0 for term in remove_filters},
            'add': {tuple(term): 0 for term in add_filters}
        }
        ...
        for idx, email in enumerate(results, start=1):
            ...
            if len(query_terms) > 1 and keyword_matches < len(query_terms) * 0.8:
            ...
            to_match = re.match(r'(.*)<(.+)>', to_field, re.IGNORECASE) if isinstance(to_field, str) else None
            if to_match:
                to_name, to_email_from_field = from_match.groups()
                email['to'] = format_email_field(to_name, to_email_from_field)
            elif to_email:
                to_name = to_field if to_field and not extract_email(to_field) else ''
                email['to'] = format_email_field(to_name, to_email)
