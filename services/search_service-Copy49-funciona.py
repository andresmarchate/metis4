...
def extract_components(explanation):
    """Extrae los componentes principales que contribuyen a la puntuación total."""
    components = {
        'text': 0.0,
        'domain': 0.0,
        'semantic': 0.0,
...
def build_explanation(total_score, components, email):
    """Construye una explicación donde las componentes suman exactamente la puntuación total."""
    explanation = f"La puntuación total de {total_score:.2f} del correo se compone de la suma de:\n"
    contributions = []
    
    # Calcular la suma total de las componentes originales
    original_sum = components['text'] + components['domain'] + components['semantic']
    
    # Si no hay componentes, indicar que la puntuación es general
    if original_sum == 0:
    ...
    # Escalar las puntuaciones para que sumen al total_score
