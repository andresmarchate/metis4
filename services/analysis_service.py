    ...
    for i in range(0, len(texts), batch_size):
        ...
        try:
            ...
            if batch_embeddings.shape[1] != 384:
                logger.error("Dimensión del embedding incorrecta: %d (esperado: 384)", batch_embeddings.shape[1])
            ...
            if 'CUDA' in str(e) and use_cpu_fallback:
                ...
                if batch_embeddings.shape[1] != 384:
                    logger.error("Dimensión del embedding incorrecta en CPU: %d (esperado: 384)", batch_embeddings.shape[1])
                    raise ValueError("Dimensión del embedding no coincide con el modelo MiniLM-L12-v2")
                embeddings.append(batch_embeddings)
...
def analyze_themes(email_ids, user):
    ...
    try:
        ...
        if n_emails < 2:
            logger.warning("Less than 2 emails, creating a single theme")
            themes = {0: {'emails': valid_emails, 'texts': texts, 'embeddings': embeddings}}
            cluster_method = "single"
        else:
            # Intentar clustering con HDBSCAN primero
            logger.debug(f"Attempting HDBSCAN clustering with {n_emails} emails")
            ...
            logger.debug(f"HDBSCAN results: {len(unique_labels)} clusters found, {noise_count} emails labeled as noise")

            if -1 not in unique_labels or noise_count == 0:
                # HDBSCAN asignó todos los correos a clusters válidos
                logger.info("HDBSCAN successfully clustered all emails")
                themes = {}
                ...
                # Fallback a KMeans debido a ruido en HDBSCAN
                logger.info(f"Falling back to KMeans due to {noise_count} emails classified as noise by HDBSCAN")
                max_k = min(10, n_emails)
                if max_k < 2:
                    max_k = 2
                best_k = 2
                best_score = -1
                ...
                logger.debug(f"Optimal k: {best_k} with silhouette score: {best_score}")
        ...
        logger.info(f"Analysis completed: {len(result)} themes identified with {sum(len(t['emails']) for t in result)} emails using {cluster_method}")
        return result
    except Exception as e:
