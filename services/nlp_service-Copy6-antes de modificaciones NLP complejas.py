...
def process_query(query, return_names=False):
    ...
    - "intent": La intención principal de la consulta. Ejemplos: "ofertas_viaje", "inversiones", "medicina_y_salud", "informacion_juridica", "negociaciones_inmobiliarias", "general". Para consultas sobre términos legales como "juzgado", "procedimiento" o "abierto", la intención debe ser "informacion_juridica". Para consultas sobre negociaciones de precios de venta de propiedades, la intención debe ser "negociaciones_inmobiliarias".
    ...
    - "metadata_filters": Diccionario con filtros de metadatos (ejemplo: {{"from": "Juan", "date_range": {{"start": "2023-01-01", "end": "2023-01-07"}}}}).
    - "names": Lista de nombres propios de personas (ejemplo: ["Teresa Goñi", "Juan Pérez"]).

    **Instrucciones**:
    1. Identifica la intención principal basándote en el propósito de la consulta. Si se mencionan términos como "negociación", "precio de venta", "casa", la intención debe ser "negociaciones_inmobiliarias".
    ...
    4. Identifica filtros de metadatos como remitente ("from"), destinatario ("to"), asunto ("subject"), y rangos de fechas ("date_range") en formato ISO 8601.
    5. Identifica nombres propios de personas en "names", excluyendo nombres de lugares o empresas.
    6. Devuelve solo un objeto JSON válido, sin comentarios ni texto adicional.

    **Ejemplos**:
    - Consulta: "que correos hay sobre negociacion con ana guerrero de precio de venta de la casa"
      Respuesta: {{"intent": "negociaciones_inmobiliarias", "terms": ["negociación", "precio de venta", "casa", "Ana Guerrero"], "conditions": {{}}, "metadata_filters": {{"from": "Ana Guerrero", "subject": "negociación sobre precio de venta de la casa"}}, "names": ["Ana Guerrero"]}}
    - Consulta: "dame todos los procedimientos del juzgado de sevilla abiertos"
      Respuesta: {{"intent": "informacion_juridica", "terms": ["procedimiento", "juzgado", "sevilla"], "conditions": {{"estado": "abiertos"}}, "metadata_filters": {{}}, "names": []}}
    ...
    elif any(word in query_lower for word in ["juzgado", "procedimiento", "abierto", "legal"]):
        intent = "informacion_juridica"
    elif any(word in query_lower for word in ["negociación", "precio de venta", "casa", "propiedad"]):
        intent = "negociaciones_inmobiliarias"

    # Post-procesamiento de nombres con regex fallback
    if not names:
