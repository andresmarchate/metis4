...
def get_email_addresses(prefix='', limit=50, user=None):
    """Fetch distinct email addresses from 'from' and 'to' fields for autocomplete, filtered by user's mailboxes."""
...
def get_conversation_emails(email1, email2, start_date, end_date, user=None):
    """Fetch bidirectional emails between email1 and email2 within a date range, filtered by user's mailboxes."""
...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False, user=None):
    ...
    try:
        ...
        # Añadir términos de búsqueda con match_phrase para precisión
        ...
        # Añadir filtro de dominio semántico
        if intent != 'general' and not filter_only:
            es_query['query']['bool']['filter'].append({
                "term": {"semantic_domain": intent}
            })
        ...
        THRESHOLD = 1.0  # Umbral de puntuación mínima
        if not es_hits or (max(hit['_score'] for hit in es_hits if '_score' in hit) < THRESHOLD):
            ...
            # Añadir puntuación de Elasticsearch a los resultados
            score_map = {hit['_source']['message_id']: hit['_score'] for hit in es_hits}
            for email in results:
                email['es_score'] = score_map.get(email['message_id'], 0)
        ...
        # Calcular relevancia con puntuación textual
        ranked_results = []
        relevance_scores = []
        semantic_scores = []
        text_scores = []
