...
def get_email_by_id(message_id):
    ...
    try:
        ...
        if not email:
            logger.warning("Correo no encontrado: %s", message_id.message)
            return None
        ...
        logger.debug("Processing to_email_field: to_field='%s', to_field=%s", to_field, email)
...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False):
    ...
    try:
        ...
        if filter_only:
            # Only return emails matching the filter terms (for filter modal)
            if not filters or not add_filters:
                logger.warning("No add filters provided for filter_only mode")
        ...
        for idx, email in enumerate(results, start=1):
            ...
            if filter_only:
                explanation_parts = [f"coincidencia con términos de filtro: {', '.join(add_filters[0])}"]
