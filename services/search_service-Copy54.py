...
def submit_bulk_feedback(query, filter_data, processed_query, intent, terms, query_embedding):
    ...
    try:
        ...
        start_time = datetime.now()
        cursor = emails_collection.find(final_conditions, {'message_id': 1}).limit(1000)  # Limitar a 1000 documentos
...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False, user=None):
    ...
    try:
        ...
        es_query = {
            ...
            "size": min(results_per_page, 100),  # Limitar a 100 resultados
        ...
        logger.debug(f"Búsqueda en Elasticsearch completada en %s segundos", (datetime.now() - search_start).total_seconds())
        ...
        # Optimización: Usar una sola consulta para contar filtros remove
        count_query_base = {
            "query": {
                "bool": {
                    "must": base_query["query"]["bool"]["must"].copy(),
        ...
        for filter in remove_filters:
            ...
            count_query = count_query_base.copy()
            count_query["query"]["bool"]["must"].extend([
                {
                    "multi_match": {
                        "query": term,
                        "fields": ["body", "summary", "relevant_terms_array", "subject", "from", "to"],
        ...
        for filter in add_filters:
            ...
            logger.debug(f"Conteo para filtro add {terms_key}: {filter_counts['add'][terms_key]} en %s segundos", (datetime.now() - count_start).total_seconds())
        ...
        logger.info("Devolviendo %s correos relevantes de %s totales en %s segundos", len(paginated_results), total_filtered_results, (datetime.now() - start_time).total_seconds())
        return {
