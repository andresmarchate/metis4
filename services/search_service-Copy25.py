...
# Version: p6q7r8s9-t0u1-v2w3-x4y5-z6a7b8c9d0
import unicodedata
import zlib
import numpy as np
...
def extract_email_from_input(input_str):
    ...
    return match.group(1) if match else extract_email(input_str)
...
def get_email_addresses(prefix='', limit=50):
    ...
    try:
        ...
        for address in all_addresses:
            ...
            match = re.match(r'(.*?)\s*(?:<(.+?)>)?$', address) if isinstance(address, str) else None
            if match:
...
def get_conversation_emails(sender, recipient, start_date, end_date):
    ...
    try:
        ...
        query = {
            '$or': [
                {
                    'from': {'$regex': sender_email, '$options': 'i'},
                    'to': {'$regex': recipient_email, '$options': 'i'}
                },
                {
                    'from': {'$regex': recipient_email, '$options': 'i'},
                    'to': {'$regex': sender_email, '$options': 'i'}
                }
            ],
            'date': {
...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False):
    ...
    try:
        ...
        if filter_only:
            ...
            # Construir consulta de texto con términos obligatorios
            text_query = {
                '$text': {
                    '$search': '"' + '" "'.join(terms) + '"',  # Require all terms
                    '$language': 'spanish'
                }
            } if terms else {}
        ...
        for idx, email in enumerate(results, start=1):
            ...
            # Ajustar pesos para mayor énfasis en coincidencia textual
            ...
            # Filtro de palabras clave para términos de consulta
            ...
            if len(query_terms) > 1 and keyword_matches < len(query_terms) * 0.8:  # Require 80% term matches
