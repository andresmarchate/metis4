...
def get_conversation_emails(email1, email2, start_date, end_date, user=None):
    ...
    try:
        ...
        pipeline = [
            {
                '$match': {
                    'mailbox_id': {'$in': user_mailboxes},  # Filtrar por buzones del usuario
                    '$or': [
                        {
...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False, user=None):
    """
    Busca correos filtrados por los buzones del usuario autenticado.
    ...
    try:
        ...
        if filter_only:
            ...
            } if terms and filter_action == 'remove' else {}
            ...
            total_results = emails_collection.count_documents({**text_query, **{'message_id': {'$exists': True}, 'mailbox_id': {'$in': user_mailboxes}}})
            ...
            for remove_terms in remove_filters:
                remove_conditions = {**text_query, 'message_id': {'$exists': True}, 'mailbox_id': {'$in': user_mailboxes}}
                normalized_terms = [normalize_text(term) for term in remove_terms]
                terms_key = ','.join(remove_terms).lower()
            ...
            for add_terms in add_filters:
                add_conditions = {'message_id': {'$exists': True}, 'mailbox_id': {'$in': user_mailboxes}}
