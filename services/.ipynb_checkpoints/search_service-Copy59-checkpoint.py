...
def search_emails(processed_query, intent, term_groups, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False, user=None, get_all_ids=False):
    ...
    try:
        ...
        # Consulta principal para resultados paginados
        es_query = {
            "query": {
                "bool": {
        ...
        # Si get_all_ids es True, obtener todos los IDs
        all_email_ids = []
        if get_all_ids:
            all_es_query = {
                "query": {
                ...
                "size": 10000,  # Límite ajustable según necesidades
                "_source": ["message_id", "index"]
            }
            for filter in remove_filters:
                for term in filter.get('terms', []):
                    all_es_query["query"]["bool"]["must_not"].append({
                        "multi_match": {
                            "query": term,
                            "fields": ["body", "summary", "relevant_terms_array", "subject", "from", "to"],
                            "type": "cross_fields"
            ...
            all_es_results = es.search(index='email_index', body=all_es_query)
            all_email_ids = [
                hit['_source']['message_id'] if hit['_source'].get('message_id') and hit['_source']['message_id'] != 'N/A'
                else hit['_source']['index']
                for hit in all_es_results['hits']['hits']
                if (hit['_source'].get('message_id') and hit['_source']['message_id'] != 'N/A') or
                   (hit['_source'].get('index') and hit['_source']['index'] != 'N/A')
            ]
