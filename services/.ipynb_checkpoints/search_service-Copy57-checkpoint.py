...
def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None, filter_only=False, user=None):
    ...
    try:
        ...
        if terms and not filter_only:
            # Separar fechas del cuerpo de otros términos
            body_date_terms = [term for term in terms if re.match(r'\d{4}-\d{2}-\d{2}', term)]
            other_terms = [term for term in terms if term not in body_date_terms]
            
            if other_terms:
                base_query["query"]["bool"]["must"].append({
                    "multi_match": {
                        "query": " ".join(other_terms),
            ...
            if body_date_terms:
                base_query["query"]["bool"]["must"].append({
                    "multi_match": {
                        "query": " ".join(body_date_terms),
                        "fields": ["body^1", "summary^1"],
                        "type": "phrase",
                        "boost": 1.5
                    }
                })
        ...
        if intent != 'general' and not filter_only:
            ...
            base_query["query"]["bool"]["minimum_should_match"] = 0  # Hacer el dominio opcional
