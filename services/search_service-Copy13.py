...
def normalize_text(text):
    ...
    if not text or not isinstance(text, str):
        return text
    # Normalizar a NFKD y eliminar diacríticos
...
def submit_bulk_feedback(query, filter_data, processed_query, intent, terms, query_embedding):
    ...
    try:
        ...
        # Construir consulta para encontrar correos que coincidan con el filtro
        filter_conditions = {'message_id': {'$exists': True}}
        ...
        # Obtener message_ids de los correos que coincidan
        cursor = emails_collection.find(filter_conditions, {'message_id': 1})
        message_ids = [email['message_id'] for email in cursor if 'message_id' in email]
        logger.debug("Encontrados %s correos para retroalimentación masiva", len(message_ids))

        affected_count = 0
        for message_id in message_ids:
            try:
                save_feedback(query, message_id, False)
                affected_count += 1
        ...
        logger.info("Retroalimentación masiva completada: %s correos afectados", affected_count)
        return affected_count
    except Exception as e:
        logger.error("Error en retroalimentación masiva: %s", str(e), exc_info=True)
        return 0

def search_emails(processed_query, intent, terms, query_embedding, min_relevance=25, page=1, results_per_page=25, filters=None):
    logger.info("Buscando correos para intent: %s, terms: %s, min_relevance: %s, page: %s, filters: %s", intent, terms, min_relevance, page, filters)
    try:
        ...
        for remove_terms in remove_filters:
            remove_conditions = {'message_id': {'$exists': True}}
            normalized_terms = [normalize_text(term) for term in remove_terms]
            remove_conditions['$or'] = [
                {
                    '$or': [
                        {'subject': {'$regex': term, '$options': 'i'}},
                        {'relevant_terms_array': {'$regex': term, '$options': 'i'}},
            ...
            remove_count = emails_collection.count_documents({**text_query, **remove_conditions})
