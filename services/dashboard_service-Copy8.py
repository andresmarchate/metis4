...
# Version: d5e6f7g8-h9i0-1234-d7e8-f9g0h1i2j3
import logging
...
def get_email_list(metric, period, sender=None, recipient=None):
    """Fetch list of emails for a specific metric and period, optionally filtered by sender or recipient."""
    logger.info(f"Fetching email list for metric: {metric}, period: {period}, sender: {sender}, recipient: {recipient}")
    try:
        now = datetime.utcnow()
        periods = {
        ...
        if sender:
            # Apply sender filter for received emails
            ...
            # Match either formatted sender or raw email
            match_criteria['from'] = {
            ...
            # Apply recipient filter for sent emails
        ...
        logger.debug("Raw email list data: %s", [{k: v for k, v in email.items() if k in ['from', 'to', 'from_email', 'to_email', 'subject']} for email in emails])
        ...
        for email in emails:
            ...
            logger.debug("Formatted email: from=%s, to=%s, subject=%s", email['from'], email['to'], email.get('subject', ''))
