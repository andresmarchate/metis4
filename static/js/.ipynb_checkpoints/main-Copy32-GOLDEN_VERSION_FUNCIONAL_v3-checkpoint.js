...
/* Version: f0g1h2i3-4j5k-6l7m-n8o9-p0q1r2s3t4 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    function escapeHtml(unsafe, isMessageId = false) {
        ...
        if (isMessageId || safe.match(/(?:^[^<]+<[^>]+@[^>]+>$|^<[^>]+@[^>]+>$|^[^<>\s]+@[^<>\s]+$)/)) {
            ...
            // Replace angle brackets with HTML entities to ensure proper rendering
            return safe.replace(/</g, '&lt;').replace(/>/g, '&gt;');
        }

        console.log('Escaping non-email text:', safe);
        // Escape all HTML characters for non-email text
        return safe
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;')
    ...
    function renderThemes(themes) {
        ...
        if (!Array.isArray(themes) || themes.length === 0) {
            ...
            themesList.innerHTML = '<p id="no-results-message">No se identificaron temas.</p>';
            currentThemes = [];
            populateThemeCheckboxes();
            showTab('themes');
            return;
