...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function parseFilterPrompt(prompt) {
        ...
        if (removeMatch) {
            ...
            console.log('Parsed remove filter terms:', terms); // Debugging
            return { action: 'remove', terms };
        } else if (addMatch) {
            const terms = addMatch[2].split(/\s*,\s*/).map(term => term.trim()).filter(term => term);
            console.log('Parsed add filter terms:', terms); // Debugging
    ...
    function renderFilters(filters) {
        ...
        filters.forEach((filter, index) => {
            ...
            li.innerText = `${actionText}: ${termsText}`;
    ...
    async function performSearch() {
        ...
        // Guardar consulta original si es nueva, sin resetear filtros
