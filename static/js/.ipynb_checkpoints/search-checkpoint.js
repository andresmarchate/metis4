const SearchModule = {
    ...
    async showEmailDetails(arg) {
        ...
        try {
            ...
            const createField = (label, value, isBody = false) => {
                ...
                if (isBody && value) {
                    ...
                    if (isHTML) {
                        ...
                        // Reemplazar enlaces con formato [image: ...] <url> por enlaces clicables
                        let formattedValue = value.replace(/\[image:[^\]]+\]\s*<([^>]+)>/g, '<a href="$1" class="email-link" target="_blank">$1</a>');
                        // Convertir URLs standalone en enlaces clicables
            ...
            createField('Términos:', email.relevant_terms ? email.relevant_terms.join(', ') : 'N/A');
                        ...
                        <table>
                            ...
                            <tbody>
                                ${data.results.map(email => {
                                    ...
                                    const isIndex = isIndex ? UtilsModule.truncateIndex(identifier) : (identifier.slice(0, 11) + '...');
