document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderThemes(themes) {
        ...
        themes.forEach(theme => {
            ...
            detail.innerHTML = `
                ...
                <p><strong>Resumen:</strong> ${escapeHtml(theme.summary)}</p>
                <div class="table-container">
                    <table>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            ${theme.emails.map(email => `
                                <tr>
                                    <td><a href="#" class="theme-email-link" data-id="${escapeHtml(email.message_id, true)}">${escapeHtml(email.message_id, true)}</a></td>
                                    <td>${escapeHtml(new Date(email.date).toLocaleString())}</td>
                                    <td>${escapeHtml(email.from)}</td>
                                    <td>${escapeHtml(email.subject)}</td>
            ...
            detail.querySelectorAll('.theme-email-link').forEach(link => {
                link.addEventListener('click', async (e) => {
                    ...
                    console.log('Clicked theme email link with message_id:', messageId);
                    if (!messageId) {
    ...
    async function performSearch() {
        ...
        try {
            ...
            currentEmailIds = results.map(result => result.message_id).filter(id => id);
    ...
    analyzeThemesBtn.addEventListener('click', async () => {
        if (!currentEmailIds || !Array.isArray(currentEmailIds) || currentEmailIds.length === 0) {
            console.warn('No valid emails to analyze', { currentEmailIds });
        ...
        // Validate email_ids
        const validEmailIds = currentEmailIds.filter(id => typeof id === 'string' && id.trim());
        if (validEmailIds.length === 0) {
            console.warn('No valid email IDs after filtering', { currentEmailIds });
            errorMessage.textContent = 'No hay IDs de correos válidos para analizar.';
            errorMessage.style.display = 'block';
            return;
        }

        try {
            console.log('Sending theme analysis request for email_ids:', validEmailIds);
            const response = await fetch('/api/analyze_themes', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email_ids: validEmailIds })
            });
    ...
    if (notRelevantBtn) {
        notRelevantBtn.addEventListener('click', async () => {
            ...
            if (!messageId) {
                console.error('No message_id found for modal feedback');
                alert('ID de correo no válido.');
                return;
            }
            try {
