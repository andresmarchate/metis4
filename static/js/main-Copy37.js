...
/* Version: e0f1g2h3-i4j5-k6l7-m8n9-o0p1q2r3s4 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    const processDeepAnalysisBtn = document.getElementById('process-deep-analysis');
    const deepAnalysisInput = document.getElementById('deep-analysis-input');
    const deepAnalysisPrompt = document.getElementById('deep-analysis-prompt');
    const submitDeepAnalysisBtn = document.getElementById('submit-deep-analysis');
    const resetDeepAnalysisContextBtn = document.getElementById('reset-deep-analysis-context');
    const deepAnalysisResponse = document.getElementById('deep-analysis-response');
    const conversationsSection = document.getElementById('conversations-section');
    const conversationsForm = document.getElementById('conversations-form');
    const email1Input = document.getElementById('email1');
    const email2Input = document.getElementById('email2');
    const startDateInput = document.getElementById('start-date');
    const endDateInput = document.getElementById('end-date');
    const errorMessageConversations = document.getElementById('error-message-conversations');
    ...
    const conversationsThemes = document.getElementById('conversations-themes');
    const analyzeConversationThemesBtn = document.getElementById('analyze-conversation-themes');
    ...
    if (!deepAnalysisSection || !themeCheckboxes || !processDeepAnalysisBtn || !deepAnalysisInput || !deepAnalysisPrompt || !submitDeepAnalysisBtn || !resetDeepAnalysisContextBtn || !deepAnalysisResponse) {
        console.error('Deep analysis elements not found');
        return;
    }
    if (!conversationsSection || !conversationsForm || !email1Input || !email2Input || !startDateInput || !endDateInput || !errorMessageConversations || !conversationsCount || !conversationsTable || !conversationsBody || !conversationsThemes || !analyzeConversationThemesBtn) {
        console.error('Conversations elements not found');
        return;
    }
    ...
    function showTab(tabId) {
        ...
        if (tabId === 'consultas') {
            if (searchSection) {
                ...
                console.log('Search section activated');
            }
            ...
            if (themesSection) {
                ...
                console.log('Themes section activated');
            }
            document.querySelector('.tab-link[data-tab="themes"]')?.classList.add('active');
        } else if (tabId === 'deep-analysis') {
            if (deepAnalysisSection) {
                deepAnalysisSection.classList.add('active');
                console.log('Deep analysis section activated');
            ...
            if (conversationsSection) {
                ...
                console.log('Conversations section activated');
            }
    ...
    function populateThemeCheckboxes() {
        console.log('Populating theme checkboxes:', currentThemes);
        themeCheckboxes.innerHTML = '';
        if (!Array.isArray(currentThemes) || currentThemes.length === 0) {
            themeCheckboxes.innerHTML = '<p>No hay temas disponibles para seleccionar.</p>';
            return;
        }
        ...
        currentThemes.forEach(theme => {
            ...
            themeCheckboxes.appendChild(checkboxWrapper);
        });
    }
                        ...
                        <table>
                            ...
                            <tbody>
                                ${data.results.map(email => `
                                    <tr>
                                        <td><a href="#" class="index-link" data-index="${escapeHtml(email.index)}">${truncateIndex(email.index)}</a></td>
                ...
                link.addEventListener('click', async (e) => {
                    ...
                    try {
                        ...
                        modalContent.innerHTML = `
                            <p><strong>Índice:</strong> ${truncateIndex(email.index)}</p>
    ...
    function updatePagination(totalResults) {
        ...
        pagination.style.display = totalResults > 0 ? 'flex' : 'none';
    }

    function renderThemes(themes, container = themesList) {
        ...
        themes.forEach(theme => {
            ...
            if (typeof theme.summary === 'object' && theme.summary.tema) {
                summaryHtml = `<ul class="theme-summary">
                    <li><strong>Tema:</strong> ${escapeHtml(theme.summary.tema)}</li>
                    <li><strong>Involucrados:</strong> ${escapeHtml(theme.summary.involucrados.join(', '))}</li>
                    <li><strong>Historia:</strong> ${escapeHtml(theme.summary.historia)}</li>
                    <li><strong>Próximos pasos:</strong> ${escapeHtml(theme.summary.proximos_pasos)}</li>
                    <li><strong>Puntos claves:</strong>
                        <ul>
                            ${theme.summary.puntos_claves.map(point => `<li>${escapeHtml(point)}</li>`).join('')}
                        </ul>
                    </li>
                </ul>`;
            ...
            detail.innerHTML = `
                <summary>
                    <strong>${escapeHtml(theme.title || 'Sin título')}</strong> (${theme.emails?.length || 0} correos, Estado: ${escapeHtml(theme.status || 'N/A')})
                </summary>
                    ...
                    <table>
                        ...
                        <tbody>
                            ${theme.emails?.map(email => {
                                ...
                                return `
                                    <tr>
                                        <td><a href="#" class="theme-email-link" data-index="${escapeHtml(index)}">${truncateIndex(index)}</a></td>
        ...
        if (container === themesList) {
            populateThemeCheckboxes();
            showTab('themes');
            console.log('Themes section should be visible after rendering');
        }
    }

    async function fetchAllSearchResults(query, minRelevance, clearCache, filters) {
        let allEmails = [];
        let page = 1;
        let totalResults = 0;
    ...
    async function performSearch() {
        ...
        try {
            ...
            resultsCount.textContent = `Se encontraron ${totalResults} correos relevantes`;
            ...
            results.forEach(result => {
                ...
                row.innerHTML = `
                    <td><a href="#" class="index-link" data-index="${escapeHtml(index)}">${truncateIndex(index)}</a></td>
    ...
    // Initialize autocomplete for email1 and email2 fields
            ...
            source: function(request, response) {
                ...
                fetch(`/api/email_addresses?prefix=${encodeURIComponent(request.term)}&limit=50`)
                    ...
                    .catch(err => {
                        ...
                        errorMessageConversations.textContent = 'Error al cargar sugerencias de correo.';
                        errorMessageConversations.style.display = 'block';
                        response([]);
                    });
            },
            minLength: 2,
    ...
    initializeAutocomplete(email1Input);
    initializeAutocomplete(email2Input);

    // Handle conversations form submission
    async function performConversationSearch() {
        ...
        if (!email1 || !email2 || !startDate || !endDate) {
            ...
            errorMessageConversations.textContent = 'Por favor, completa todos los campos.';
            errorMessageConversations.style.display = 'block';
            return;
        }

        errorMessageConversations.style.display = 'none';
        conversationsBody.innerHTML = '';
        conversationsTable.style.display = 'none';
        conversationsCount.style.display = 'none';
        analyzeConversationThemesBtn.style.display = 'none';
        conversationsThemes.innerHTML = '';
        ...
        try {
            ...
            if (results.length === 0) {
                ...
                errorMessageConversations.textContent = 'No se encontraron correos para los criterios seleccionados.';
                errorMessageConversations.style.display = 'block';
                return;
            }

            conversationsCount.textContent = `Se encontraron ${totalResults} correos`;
            conversationsCount.style.display = 'block';
            conversationsTable.style.display = 'table';
            analyzeConversationThemesBtn.style.display = 'block';

            currentConversationEmails = results.map(result => ({
                message_id: result.message_id,
                index: result.index
            })).filter(email => email.index || email.message_id);
            ...
            results.forEach(result => {
                ...
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td><a href="#" class="index-link" data-index="${escapeHtml(result.index)}">${truncateIndex(result.index)}</a></td>
                    <td>${escapeHtml(result.date ? new Date(result.date).toLocaleString() : 'N/A')}</td>
                    <td>${escapeHtml(result.from || 'N/A')}</td>
                    <td>${escapeHtml(result.to || 'N/A')}</td>
                    <td>${escapeHtml(result.subject || '')}</td>
                    <td>${escapeHtml((result.description || '').slice(0, 100))}${result.description && result.description.length > 100 ? '...' : ''}</td>
            ...
            // Attach click handlers for index links
            conversationsBody.querySelectorAll('.index-link').forEach(link => {
                link.addEventListener('click', async (e) => {
                    e.preventDefault();
                    const identifier = e.target.dataset.index;
            ...
            errorMessageConversations.textContent = 'Error al buscar correos: ' + err.message;
            errorMessageConversations.style.display = 'block';
        }
    }

    // Handle conversation theme analysis
    analyzeConversationThemesBtn.addEventListener('click', async () => {
        console.log('Analyze conversation themes clicked');
        if (!currentConversationEmails || !Array.isArray(currentConversationEmails) || currentConversationEmails.length === 0) {
            console.warn('No valid conversation emails to analyze', { currentConversationEmails });
            errorMessageConversations.textContent = 'No hay correos válidos para analizar temas.';
            errorMessageConversations.style.display = 'block';
            return;
        }

        const emailIds = currentConversationEmails.map(email => email.index || email.message_id).filter(id => id && id !== 'N/A');
        if (emailIds.length === 0) {
            console.warn('No valid email identifiers for theme analysis', { emailIds });
            errorMessageConversations.textContent = 'No hay identificadores de correos válidos para analizar.';
            errorMessageConversations.style.display = 'block';
            return;
        }
        ...
        try {
            ...
            renderThemes(data.themes || [], conversationsThemes);
            errorMessageConversations.style.display = 'none';
        } catch (error) {
            console.error('Error analyzing conversation themes:', error.message);
            errorMessageConversations.textContent = `Error al analizar temas: ${error.message}`;
            errorMessageConversations.style.display = 'block';
            conversationsThemes.innerHTML = '<p id="no-results-message">No se identificaron temas.</p>';
        }
    });
    ...
    analyzeThemesBtn.addEventListener('click', async () => {
        ...
        const validEmails = await fetchAllSearchResults(currentQuery, currentMinRelevance, clearCacheInput.checked, JSON.parse(sessionStorage.getItem('filters') || '[]'));
        if (validEmails.length === 0) {
            console.warn('No valid email identifiers after fetching all results', { validEmails });
        ...
        try {
            console.log('Sending theme analysis request for all emails:', validEmails);
            const response = await fetch('/api/analyze_themes', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email_ids: validEmails.map(email => email.index || email.message_id) })
            });
    ...
    processDeepAnalysisBtn.addEventListener('click', async () => {
        console.log('Process deep analysis clicked');
        const selectedThemes = Array.from(themeCheckboxes.querySelectorAll('input[type="checkbox"]:checked'))
            .map(checkbox => ({
        ...
        try {
            ...
            deepAnalysisSessionId = data.session_id;
            deepAnalysisEmails = data.email_data || [];
            deepAnalysisInput.style.display = 'block';
            deepAnalysisResponse.innerHTML = `
                ...
                <div class="table-container">
                    <table>
                        ...
                        <tbody>
                            ${deepAnalysisEmails.map(email => `
                                <tr>
                                    <td><a href="#" class="deep-analysis-email-link" data-index="${escapeHtml(email.index)}">${truncateIndex(email.index)}</a></td>
                                    <td>${escapeHtml(email.date ? new Date(email.date).toLocaleString() : 'N/A')}</td>
                                    <td>${escapeHtml(email.from)}</td>
                                    <td>${escapeHtml(email.to)}</td>
                                    <td>${escapeHtml(email.subject)}</td>
                                    <td>${escapeHtml(email.summary.slice(0, 100))}${email.summary.length > 100 ? '...' : ''}</td>
                                </tr>
    ...
    function attachDeepAnalysisEmailListeners() {
    ...
    submitDeepAnalysisBtn.addEventListener('click', async () => {
        const prompt = deepAnalysisPrompt.value.trim();
        ...
        try {
            const response = await fetch('/api/deep_analysis_prompt', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ session_id: deepAnalysisSessionId, prompt })
            });
            ...
            const linkedReasoning = data.reasoning.replace(indexRegex, (match, index) => {
                ...
                return `<a href="#" class="deep-analysis-email-link" data-index="${index}">${truncateIndex(index)}</a>`;
            });
            ...
            deepAnalysisResponse.innerHTML = `
                ...
                <p>${linkedReasoning}</p>
                <h3>Alternativas:</h3>
                <ul>${data.alternatives.map(alt => `<li>${escapeHtml(alt)}</li>`).join('') || '<li>Ninguna</li>'}</ul>
                <h3>Referencias:</h3>
                <div class="table-container">
                    <table>
                        <thead>
                        ...
                        <tbody>
                            ${data.references.map(ref => `
                                <tr>
                                    <td><a href="#" class="deep-analysis-email-link" data-index="${escapeHtml(ref.index)}">${truncateIndex(ref.index)}</a></td>
                                    <td>${escapeHtml(ref.date ? new Date(ref.date).toLocaleString() : 'N/A')}</td>
                                    <td>${escapeHtml(ref.from)}</td>
                                    <td>${escapeHtml(ref.to)}</td>
                                    <td>${escapeHtml(ref.subject)}</td>
                                    <td>${escapeHtml(ref.summary.slice(0, 100))}${ref.summary.length > 100 ? '...' : ''}</td>
                                </tr>
    ...
    resetDeepAnalysisContextBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('/api/deep_analysis_reset', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ session_id: deepAnalysisSessionId })
            });
            ...
            deepAnalysisSessionId = null;
            deepAnalysisEmails = [];
            deepAnalysisInput.style.display = 'none';
            deepAnalysisPrompt.value = '';
            deepAnalysisResponse.innerHTML = '<p>Contexto reseteado. Selecciona nuevos temas para continuar.</p>';
    ...
    if (applyFilterBtn) {
        applyFilterBtn.addEventListener('click', () => {
            ...
            console.log('Aplicando filtro con prompt:', prompt);
            const filter = parseFilterPrompt(prompt);
            if (!filter || !filter.terms || filter.terms.length === 0) {
                console.warn('Prompt de filtro inválido:', prompt);
                errorMessage.textContent = 'Prompt inválido. Ejemplo: "elimina correos que incluyan reunión, proyecto"';
                errorMessage.style.display = 'block';
                return;
            }
    ...
    if (closeModal) {
        closeModal.addEventListener('click', () => {
            console.log('Cerrando modal de correo');
            modal.style.display = 'none';
        });
    }
    ...
    window.addEventListener('click', (e) => {
        if (e.target === modal) {
            console.log('Cerrando modal de correo por clic en fondo');
            modal.style.display = 'none';
        }
    });
    ...
    if (notRelevantBtn) {
        notRelevantBtn.addEventListener('click', async () => {
            ...
            console.log('Marcando correo como no relevante desde modal:', identifier);
            if (!identifier || identifier === 'N/A') {
                console.error('No se encontró un identificador válido para retroalimentación en modal');
                alert('Identificador de correo no válido.');
                return;
            }
            try {
                console.log('Enviando retroalimentación para identificador:', identifier);
                const response = await fetch('/api/feedback', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                ...
                console.log('Retroalimentación enviada correctamente');
                alert('Retroalimentación enviada.');
                modal.style.display = 'none';
            } catch (err) {
    ...
    console.log('Filtros iniciales al cargar:', initialFilters);
