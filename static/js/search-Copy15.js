    ...
    init({ currentQuery, currentMinRelevance, currentPage, resultsPerPage, filterCounts, currentEmails, setCurrentEmails, setFilterCounts, setTotalPages }) {
        ...
        this.analyzeThemesBtn.addEventListener('click', async () => {
            ...
            const emailIds = this.currentEmails
                .map(email => email.message_id || email.index)
                        ...
                        <table>
                            ...
                            <tbody>
                                ${data.results.map(email => {
                                    const identifier = (email.index && email.index !== 'N/A') ? email.index : email.message_id;
                                    const isIndex = (email.index && email.index !== 'N/A');
                                    console.log(`Email identifier: ${identifier}, isIndex: ${isIndex}`);
                                    return `
                                        <tr>
                                            <td><a href="#" class="index-link" data-identifier="${UtilsModule.escapeHtml(identifier)}" data-is-index="${isIndex}">${UtilsModule.truncateIndex(identifier)}</a></td>
