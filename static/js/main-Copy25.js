...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentThemes = []; // Array of themes from analysis
    ...
    if (!deepAnalysisSection || !themeCheckboxes || !processDeepAnalysisBtn) {
    ...
    function renderThemes(themes) {
        ...
        if (!Array.isArray(themes) || themes.length === 0) {
            ...
            showTab('themes');
            console.log('Themes section should be visible after no themes');
            return;
        }

        currentThemes = themes; // Store themes for deep analysis
    ...
    processDeepAnalysisBtn.addEventListener('click', () => {
        ...
        // Mock action: Show prompt input (backend integration in Step 3)
        const deepAnalysisInput = document.getElementById('deep-analysis-input');
        if (deepAnalysisInput) {
            deepAnalysisInput.style.display = 'block';
            document.getElementById('deep-analysis-response').innerHTML = '<p>Análisis inicializado para los temas seleccionados. Ingresa tu consulta.</p>';
        }
    });
