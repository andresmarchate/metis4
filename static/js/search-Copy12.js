const SearchModule = {
    init({ currentQuery, currentMinRelevance, currentPage, resultsPerPage, filterCounts, currentEmails, setCurrentEmails, setFilterCounts, setTotalPages }) {
        ...
        this.analyzeThemesBtn.addEventListener('click', async () => {
            ...
            const emailIds = this.currentEmails
                .map(email => email.index !== 'N/A' ? email.index : email.message_id)
    ...
    async performSearch() {
        ...
        try {
            ...
            results.forEach(result => {
                ...
                console.log('Rendered description cell:', descCellleansCell.textContent);
    ...
    async showFilterEmailsModal(filter) {
        ...
        try {
            const response = await fetch('/api/filter_emails', {
                ...
                body: JSON.stringify({
                    filter: filter,
                    user: 'andres.marchante'  // Ajustar según el contexto del usuario en tu aplicación
                })
            });
