...
/* Version: x4y5z6a7-8901-2345-x6y7-z89012345678 */
const DeepAnalysisModule = {
    ...
    populateThemeCheckboxes(themes = ThemesModule.currentThemes) {
        ...
        // Store current selected theme IDs
    ...
    // Utility to format email addresses for display
    formatEmailAddress(email) {
        if (!email || email === 'N/A') return 'N/A';
        // Remove angle brackets and extra quotes
        let cleaned = email.replace(/^<|>$/g, '').replace(/^['"]+|['"]+$/g, '');
        // Extract name and email if in "Name <email>" format
        const match = cleaned.match(/(.*?)\s*(?:<([\w\.-]+@[\w\.-]+\.\w+)>)?$/);
        if (match) {
            const name = match[1]?.trim();
            const addr = match[2]?.trim();
            if (name && addr) {
                return `${UtilsModule.escapeHtml(name)} (${UtilsModule.escapeHtml(addr)})`;
            } else if (addr) {
                return UtilsModule.escapeHtml(addr);
            } else if (name) {
                return UtilsModule.escapeHtml(name);
            }
        }
        // Fallback for simple email or malformed input
        return UtilsModule.escapeHtml(cleaned);
    },
    ...
    async processDeepAnalysis() {
        ...
        // Only activate tab if not already active
                ...
                ? this.deepAnalysisEmails.map((email, index) => {
                    ...
                    return `
                        <tr>
                            ...
                            <td>${this.formatEmailAddress(email.from)}</td>
                            <td>${this.formatEmailAddress(email.to)}</td>
                            <td>${UtilsModule.escapeHtml(email.subject || 'N/A')}</td>
                            <td>${UtilsModule.escapeHtml((email.summary || '').slice(0, 100))}${email.summary && email.summary.length > 100 ? '...' : ''}</td>
                        </tr>
                    `;
                }).join('')
                        ...
                        <tbody>
                            ${data.references.map(ref => `
                                <tr>
                                    ...
                                    <td>${this.formatEmailAddress(ref.from)}</td>
                                    <td>${this.formatEmailAddress(ref.to)}</td>
                ...
                try {
                    ...
                    if (detailsElement) {
                        detailsElement.innerHTML = `
                            ...
                            <p><strong>De:</strong> ${this.formatEmailAddress(email.from)}</p>
                            <p><strong>Para:</strong> ${this.formatEmailAddress(email.to)}</p>
