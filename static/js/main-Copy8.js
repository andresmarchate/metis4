...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const clearCacheInput = document.getElementById('clear-cache'); // Nuevo
    ...
    function escapeHtml(unsafe, isMessageId = false) {
        ...
        // Si es message_id, no escapar < y >
        if (isMessageId && safe.includes('@') && safe.startsWith('<') && safe.endsWith('>')) {
            return safe; // Preservar message_id como está
        }

        // Detectar formato de correo "Nombre <email>" o "<email>"
        const emailRegex = /^([^<]*?)\s*(<[^>]+>)?$/;
        const match = safe.match(emailRegex);
        if (match) {
            const namePart = match[1].trim();
            const emailPart = match[2] || ''; // Puede ser vacío si no hay <email>
            const escapedName = namePart
                .replace(/&/g, "&")
                .replace(/"/g, "") // Corregido: reemplazar comillas con nada
                .replace(/'/g, "'");
            // Devolver nombre escapado + email sin escapar (si existe)
            return emailPart ? `${escapedName} ${emailPart}`.trim() : escapedName || 'N/A';
        }

        // Escapar todo para otros casos
        return safe
            .replace(/&/g, "&")
            .replace(/</g, "<")
            .replace(/>/g, ">")
    ...
    function formatEmail(field) {
        ...
        // Si ya tiene formato correcto o contiene <email>, devolver tal cual
        if (field.match(/<[^>]+>/) || field.match(/^[^<]+?\s*<[^>]+>$/) || field.match(/^<[^>]+>$/)) {
            return field;
        }
        // Si es solo un correo sin formato, envolver en < >
        if (field.includes('@') && !field.includes('<')) {
            return `<${field}>`;
        }
        // Si es solo nombre, devolver como está
        return field;
    }
    ...
    async function performSearch() {
        ...
        const clearCache = clearCacheInput.checked; // Obtener estado del checkbox
        currentQuery = query;
        ...
        try {
            ...
            const response = await fetch('/api/search', {
                ...
                body: JSON.stringify({
                    ...
                    page: currentPage,
                    resultsPerPage,
                    clearCache // Enviar estado del checkbox
                })
            });
