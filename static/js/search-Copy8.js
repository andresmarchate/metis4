const SearchModule = {
    init({ currentQuery, currentMinRelevance, currentPage, resultsPerPage, filterCounts, currentEmails, setCurrentEmails, setFilterCounts, setTotalPages }) {
        ...
        this.analyzeThemesBtn.addEventListener('click', async () => {
            ...
            const validEmailIds = emailIds
                .map(email => email.index || email.message_id)
                .filter(id => id && id !== 'N/A' && typeof id === 'string');
            console.log('Filtered valid email IDs:', validEmailIds);
            ...
            try {
                console.log('Sending theme analysis request for emails:', validEmailIds);
    ...
    async fetchAllSearchResults() {
        ...
        return allEmails.filter(email => (email.index && email.index !== 'N/A') || (email.message_id && email.message_id !== 'N/A'));
    },
    ...
    async performSearch() {
        ...
        try {
            ...
            const newEmails = results.map(result => ({
                message_id: result.message_id,
                index: result.index
            })).filter(email => (email.index && email.index !== 'N/A') || (email.message_id && email.message_id !== 'N/A'));
    ...
    updatePagination(totalResults) {
        this.setTotalPages(Math.ceil(totalResults / this.resultsPerPage) || 1);
                        ...
                        <table>
                            ...
                            <tbody>
                                ${data.results.map(email => `
                                    <tr>
                                        <td><a href="#" class="index-link" data-identifier="${UtilsModule.escapeHtml(email.index || email.message_id)}" data-is-index="${email.index && email.index !== 'N/A'}">${UtilsModule.truncateIndex(email.index || email.message_id)}</a></td>
