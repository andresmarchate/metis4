const SearchModule = {
    init({ currentQuery, currentMinRelevance, currentPage, resultsPerPage, filterCounts, currentEmails, setCurrentEmails, setFilterCounts, setTotalPages }) {
        ...
        this.totalPages = 1;
        this.searchResults = []; // Almacenar resultados de búsqueda para usar en la modal
        ...
        this.analyzeThemesBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (typeof ThemesModule !== 'undefined') {
                    ...
                    // Simular clic en la pestaña "Análisis de Temas" para mostrar el contenido
                    const themesTabLink = document.querySelector('.tab-link[data-tab="themes"]');
                    if (themesTabLink) {
                        themesTabLink.click();
                    } else {
    ...
    async performSearch(attempt = 1, maxAttempts = 2) {
        ...
        try {
            ...
            this.searchResults = results; // Guardar resultados para usar en la modal
            const totalResults = data.totalResults || 0;
            const allEmailIds = data.all_email_ids || [];
            ...
            if (!Array.isArray(results) || results.length === 0) {
                ...
                this.resultsBody.querySelectorAll('.not-relevant').forEach(button => {
                    button.addEventListener('click', async (e) => {
                        ...
                        console.log('Clicked not-relevant button', identifier);
    ...
    async showEmailDetails(arg) {
        ...
        const emailFromSearch = this.searchResults.find(result => 
            (isIndex && result.index === identifier) || (!isIndex && result.message_id === identifier)
        );
        ...
        try {
            ...
            const attachmentsContent = Array.isArray(email.attachments_content) ? email.attachments_content.join('\n') : email.attachments_content || '';
            ...
            createField('Términos:', emailFromSearch && Array.isArray(emailFromSearch.relevant_terms) ? emailFromSearch.relevant_terms.join(', ') : 'N/A');
            createField('Explicación:', emailFromSearch && emailFromSearch.explanation ? emailFromSearch.explanation : 'N/A');
            ...
            alert('Error al cargar los detalles del correo: ' + (err.message || 'Error desconocido'));
        }
    },
