...
/* Version: d4e5f6g7-1h8i-9j0k-l1m2-3n4o5p6q7r8s */
document.addEventListener('DOMContentLoaded', () => {
    ...
    submitDeepAnalysisBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            const indexRegex = /\'([0-9a-f]{64})\'/g;
            console.log('Reasoning text before processing:', data.reasoning);
            const linkedReasoning = escapeHtml(data.reasoning).replace(indexRegex, (match, index) => {
                console.log('Found index in reasoning:', index);
                return `<a href="#" class="deep-analysis-email-link" data-index="${escapeHtml(index)}">${truncateIndex(index)}</a>`;
            });
