document.addEventListener('DOMContentLoaded', () => {
    ...
    function truncateIndex(index) {
        // Display first 8 characters of the index for readability
    ...
    analyzeThemesBtn.addEventListener('click', async () => {
        if (!currentQuery || !Array.isArray(currentEmails) || currentEmails.length === 0) {
            console.warn('No valid emails to analyze', { currentEmails });
            errorMessage.textContent = 'No hay correos disponibles para analizar temas.';
            errorMessage.style.display = 'block';
            return;
        }

        try {
            console.log('Fetching all emails for theme analysis...');
            ...
            const response = await fetch('/api/analyze_themes', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ email_ids: validEmails.map(email => email.index) })
            });
