...
const ThreadsModule = {
    ...
    renderThreads(threads) {
        ...
        threads.forEach((thread, threadIndex) => {
            ...
            const coherence = thread.coherence ? thread.coherence.toFixed(2) : 'N/A';
            const avgConcordance = thread.emails.length > 0 
                ? (thread.emails.reduce((sum, email) => sum + email.concordance, 0) / thread.emails.length).toFixed(2) 
                : 'N/A';
            ...
            thread.emails.forEach(email => {
                ...
                dateCell.textContent = UtilsModule.escapeHtml(email.date);
                ...
                confidenceCell.textContent = email.confidence_score.toFixed(2);
                row.appendChild(confidenceCell);

                // Concordancia
                const concordanceCell = document.createElement('td');
                concordanceCell.textContent = email.concordance.toFixed(2);
