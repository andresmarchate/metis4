...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!filtersList) {
        console.error('Filters list element not found in DOM');
    }
    ...
    function renderFilters(filters) {
        console.log('Rendering filters:', filters); // Debugging
        if (!filtersList) {
        ...
        filters.forEach((filter, index) => {
            console.log('Creating filter item:', { filter, index }); // Debugging
            const li = document.createElement('li');
            const actionText = filter.action === 'remove' ? 'Eliminar' : 'Añadir';
            const termsText = escapeHtml(filter.terms.join(', '));
            li.innerText = `${actionText}: ${termsText}`; // Use innerText to avoid innerHTML issues
        ...
        console.log('Filters list updated:', filtersList.innerHTML); // Debugging
    }

    // Eliminar un filtro y recargar resultados
    function removeFilter(index) {
        console.log('Removing filter at index:', index); // Debugging
    ...
    async function performSearch() {
        ...
        console.log('Performing search with filters:', filters); // Debugging
    ...
    if (applyFilterBtn) {
        applyFilterBtn.addEventListener('click', () => {
            ...
            console.log('Applying filter with prompt:', prompt); // Debugging
            ...
            let filters = JSON.parse(sessionStorage.getItem('filters') || '[]');
            filters.push(filter);
            console.log('Updated filters:', filters); // Debugging
            sessionStorage.setItem('filters', JSON.stringify(filters));
            renderFilters(filters);
            filterPrompt.value = '';
            performSearch();
    ...
    console.log('Initial filters on load:', initialFilters); // Debugging
    renderFilters(initialFilters);
});