...
/* Version: i9j0k1l2-3456-7890-i1j2-k34567890123 */
const DeepConversationAnalysisModule = {
    init({ deepConversationAnalysisSessionId, deepConversationAnalysisEmails, setDeepConversationAnalysisSessionId, setDeepConversationAnalysisEmails }) {
        ...
        console.log('DeepConversationAnalysisModule init:', {
            deepConversationAnalysisSection: !!this.deepConversationAnalysisSection,
            conversationThemeCheckboxes: !!this.conversationThemeCheckboxes,
            processDeepConversationAnalysisBtn: !!this.processDeepConversationAnalysisBtn,
            deepConversationAnalysisInput: !!this.deepConversationAnalysisInput,
            deepConversationAnalysisResponse: !!this.deepConversationAnalysisResponse
        });

        // Event listeners
        if (this.processDeepConversationAnalysisBtn) {
            this.processDeepConversationAnalysisBtn.addEventListener('click', () => this.processDeepConversationAnalysis());
        } else {
            console.warn('processDeepConversationAnalysisBtn not found');
        }
        if (this.submitDeepConversationAnalysisBtn) {
            this.submitDeepConversationAnalysisBtn.addEventListener('click', () => this.submitDeepConversationAnalysis());
        }
        if (this.resetDeepConversationAnalysisContextBtn) {
            this.resetDeepConversationAnalysisContextBtn.addEventListener('click', () => this.resetDeepConversationAnalysisContext());
        }

        console.log('DeepConversationAnalysisModule initialized successfully');
    },

    showTab() {
        console.log('Showing Deep Conversation Analysis tab');
        ...
        const tabLink = document.querySelector('.tab-link[data-tab="deep-conversation-analysis"]');
        if (tabLink) {
            tabLink.classList.add('active');
        } else {
            console.warn('Tab link for "deep-conversation-analysis" not found');
        }
    ...
    populateConversationThemeCheckboxes() {
        ...
        if (!Array.isArray(ConversationsModule.currentConversationThemes) || ConversationsModule.currentConversationThemes.length === 0) {
            console.warn('No conversation themes available to populate checkboxes');
        ...
        ConversationsModule.currentConversationThemes.forEach(theme => {
            console.log('Theme details:', {
                ...
                title: theme.title,
                email_count: theme.emails ? theme.emails.length : 0
            });
    ...
    async processDeepConversationAnalysis() {
        ...
        // Get conversation context from ConversationsModule
        const email1 = ConversationsModule.email1Input?.value.trim();
        const email2 = ConversationsModule.email2Input?.value.trim();
        const startDate = ConversationsModule.startDateInput?.value;
        const endDate = ConversationsModule.endDateInput?.value;

        console.log('Conversation context:', { email1, email2, startDate, endDate });

        if (!email1 || !email2 || !startDate || !endDate) {
            console.error('Missing conversation context:', { email1, email2, startDate, endDate });
            alert('Error: Falta información de contexto de la conversación. Por favor, realiza una búsqueda en la pestaña de Conversaciones primero.');
            return;
        }

        try {
            console.log('Sending POST request to /api/deep_conversation_analysis_init', {
                email1,
                email2,
                start_date: startDate,
                end_date: endDate,
                theme_ids: selectedThemes.map(theme => theme.theme_id)
            });
            ...
            if (!data.themes || data.themes.length === 0) {
                console.warn('No themes returned in initialization response');
                alert('No se encontraron temas para los criterios seleccionados. Intenta con otros temas o verifica la búsqueda.');
                return;
            }

            this.setDeepConversationAnalysisSessionId(data.session_id);
            console.log('Session ID set:', this.deepConversationAnalysisSessionId);
    ...
    async submitDeepConversationAnalysis() {
        ...
        if (!this.deepConversationAnalysisSessionId) {
            console.error('No active session found:', this.deepConversationAnalysisSessionId);
            alert('No hay una sesión activa. Por favor, inicializa el análisis primero.');
            return;
        }

        try {
            console.log('Sending POST request to /api/deep_conversation_analysis_prompt', { session_id: this.deepConversationAnalysisSessionId, prompt });
                        ...
                        <tbody>
                            ${data.references.map(ref => `
                                <tr>
                                    ...
                                    <td>${UtilsModule.escapeHtml(ref.subject || 'N/A')}</td>
    ...
    async resetDeepConversationAnalysisContext() {
        ...
        try {
            console.log('Sending POST request to /api/deep_conversation_analysis_reset', { session_id: this.deepConversationAnalysisSessionId });
    ...
    attachDeepConversationEmailListeners() {
        const links = this.deepConversationAnalysisResponse ? this.deepConversationAnalysisResponse.querySelectorAll('.deep-conversation-email-link') : [];
