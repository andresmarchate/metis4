...
const DashboardModule = {
    ...
    showTab() {
        ...
        this.loadMetrics();
        this.loadTodos(); // Añadido para AGATTA
    },
    ...
    async loadTodos() {
    console.log('Loading AGATTA todos');
    if (!this.todosList) {
        console.error('Cannot load todos: todosList is null');
        return;
    }
    try {
        const response = await fetch('/api/agatta/todos', {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        });
        if (response.status === 401) {
        ...
        console.log('Todos received:', todos); // Añadir este log para depurar
        this.renderTodos(todos);
    } catch (error) {
        console.error('Error loading AGATTA todos:', error.message);
    ...
    this.todosList.innerHTML = `
        ...
        ${todos.map(todo => `
            <div class="todo-item">
                ...
                <span>${todo.subject || 'Sin Asunto'}</span> <!-- Usar subject en lugar de task -->
            </div>
        `).join('')}
    `;
    },
...
console.log("Usuario autenticado:", current_user ? current_user.username : "No definido");
