    ...
    addResetCacheButton() {
        ...
        resetButton.addEventListener('click', async () => {
            ...
            const emailIds = this.currentThemes.flatMap(theme => 
                theme.emails.map(email => String(email.index || email.message_id))
            ).filter(id => id && id !== 'N/A');
            console.log('Email IDs to send for cache reset:', emailIds);
            if (!emailIds.length) {
                console.warn('No valid email IDs available to reset cache');
                alert('No hay temas actuales con IDs válidos para resetear el caché.');
                return;
            }
            try {
