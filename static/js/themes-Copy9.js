...
const ThemesModule = {
    init({ currentThemes, setCurrentThemes }) {
        ...
        // Añadir botón de resetear caché
    ...
    addResetCacheButton() {
        ...
        resetButton.addEventListener('click', async () => {
            ...
            const emailIds = this.currentThemes.flatMap(theme => theme.emails.map(email => String(email.index)));
            console.log('Email IDs to send for cache reset:', emailIds); // Log para depuración
            if (!emailIds.length) {
                console.warn('No email IDs available to reset cache');
                alert('No hay temas actuales para resetear el caché.');
                return;
            }
            try {
                ...
                this.renderThemes(this.currentThemes); // Refrescar temas actuales
            } catch (error) {
