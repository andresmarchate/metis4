    ...
    async showEmailDetails(arg) {
        ...
        try {
            ...
            const createField = (label, value, isBody = false) => {
                const p = document.createElement('div'); // Usamos div para mejor control de estilo
                ...
                if (isBody && value) {
                    ...
                    // Detectar si el contenido parece ser HTML
                    const isHTML = /<[a-z][\s\S]*>/i.test(value);
                    if (isHTML) {
                        // Crear un contenedor temporal para sanitizar HTML
                        const tempDiv = document.createElement('div');
                        tempDiv.innerHTML = value;
                        // Eliminar scripts y atributos potencialmente peligrosos
                        tempDiv.querySelectorAll('script, [on*]').forEach(el => el.remove());
                        bodyDiv.appendChild(tempDiv);
                    } else {
                        // Para texto plano, preservar espacios y saltos de línea
                        bodyDiv.style.whiteSpace = 'pre-wrap';
                        bodyDiv.textContent = value;
                    }
