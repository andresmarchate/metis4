...
# Version: g0h1i2j3-k4l5-m6n7-o8p9-q0r1s2t3u4
...
def extract_features(email, terms, names, query):
    ...
    query_vector = tfidf_vectorizer.transform([query or "default_query"])
    ...
    subject_score = cosine_similarity(query_vector, tfidf_vectorizer.transform([email.get('subject', '') or "default_subject"]))[0][0]
...
def initialize_bayesian_filter():
    ...
    try:
        ...
        logger.info(f"Processed {group_count} groups, collected {len(texts)} texts, {len(queries)} queries, {empty_text_emails} empty-text emails")
        
        # Fit TF-IDF vectorizer
        all_texts = texts + queries if texts or queries else ["default_text"]
        ...
        if features:
            ...
            logger.warning("No sufficient data for training, initializing default weights")
            default_weights = {'summary': 0.2, 'terms': 0.2, 'subject': 0.2, 'thread_id': 0.2, 'names': 0.2}
            feedback_collection.update_one(
                {'type': 'weights'},
                {'$set': {'weights': default_weights, 'timestamp': datetime.utcnow()}},
                upsert=True
            )
            logger.info(f"Default weights stored: {default_weights}")
